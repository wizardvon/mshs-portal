...
export function ExcelImportButton<T>({
  ...
  return (
    <div className="rounded-2xl border border-dashed border-red-200 bg-white p-4 shadow-sm shadow-slate-200/70">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button
          className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl border border-red-100 bg-red-50 px-4 text-sm font-bold text-civic transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
      ...
      {message && <p className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-700">{message}</p>}
      {error && <p className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-medium text-red-700">{error}</p>}
      <input
        accept=".xlsx,.xls,.csv"
        className="hidden"
        onChange={(event) => {
